...
{
    //非阻塞且不被子进程继承
    // CLOEXEC: CLOSE ON EXEC => 在exec时关闭
...
EventLoop::EventLoop()
    : looping_(false), quit_(false), calling_pending_functors_(false), threadId_(Current_thread::tid()), poller_(Poller::new_defaultPoller(this)),
